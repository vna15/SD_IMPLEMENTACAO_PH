...
;*********** SETUP INÍCIO DO RTC **************
SETUP_LOOP:
	LDI		r17, (1<<DDB3) | (1<<DDB5) | (1<<DDB2) | (1<<DDB1) | (1<<DDB0) | (1<<DDB7)
	OUT		DDRB, r17

	LDI		r24, 0x59 ; 59min.
	LDI		r25, 0x23 ; 23horas
	LDI		r26, 0x03 ; Terça-Feira
	RCALL RTC_SETUP	; Setup Hora, alarmes e interrupção

	LDI		r24, 0x00 ; 59min.
	LDI		r25, 0x00 ; 12horas
	LDI		r26, 0x04 ; Terça-Feira
...
loop:
	...
	check_flag:
		;in r16, PIND3
		;cpi r16, 0
		rcall rtc_get_alarms
		cpi r16, 201
		breq loop_exec_clr_flag1
		cpi r16, 202
		breq loop_exec_clr_flag2
		rjmp loop_cp_check_buttons
	loop_exec_clr_flag1:
	    sbi PORTC, PC5
		rcall delay
		;ldi r16, (1<<PB7)
...
loop_Off_H:
	...
	rcall delay
	sbic PINC, PC2
	rcall seta_cima_H_A_2
	sbic PINC, PC3
	rcall seta_baixo_H_A_2
 	;rcall rtc_set_H
	sbis PINC, PC1
	rjmp loop_Off_H
	rcall delay
...
loop_Off_M:
	...
	rcall delay
	sbic PINC, PC2
	rcall seta_cima_M_A_2
	sbic PINC, PC3
	rcall seta_baixo_M_A_2
	sbis PINC, PC1
	rjmp loop_Off_M
	rcall delay
	rcall incrementeB
...
loop_Week_Off:
	...
	rcall delay
	sbic PINC, PC2
	rcall seta_cima_D_A_2
	sbic PINC, PC3
	rcall seta_baixo_D_A_2
	sbis PINC, PC1
	rjmp loop_Week_Off
	rcall delay
	rcall incrementeB
...
seta_cima_H_A_2:
	...
	breq hora_min_A2
	rcall soma_Hexa_H
	rjmp seta_cima_hora_end_A2
hora_min_A2:
	ldi r25, 0x00
seta_cima_hora_end_A2:
	RCALL RTC_SET_H_A2	
	ret
;-------------------------------------------------
;*************** seta_cima_M_A ********************
...
seta_cima_M_A_2:
	...
	breq minuto_min_A2
	rcall soma_Hexa_M
	rjmp seta_cima_minuto_end_A2
minuto_min_A2:
	ldi r24, 0x00	
seta_cima_minuto_end_A2:
	RCALL RTC_SET_M_A2
	ret
;-------------------------------------------------
;*************** seta_cima_D_A ********************
...
seta_cima_D_A_2:
	...
	breq dia_min_A2
	inc r26
	rjmp seta_cima_dia_end_A2
dia_min_A2:
	ldi r26, 0x01	
seta_cima_dia_end_A2:
	RCALL RTC_SET_D_A2
	ret
;-------------------------------------------------
;*************** seta_baixo_H_A ********************
...
seta_baixo_H_A_2:
	...
	breq hora_max_A2
	rcall subtrai_Hexa_H
	rjmp seta_baixo_hora_end_A2
hora_max_A2:
	ldi r25, 0x23	
seta_baixo_hora_end_A2:
	RCALL RTC_SET_H_A2
	ret
;-------------------------------------------------
;*************** seta_baixo_M_A ********************
...
seta_baixo_M_A_2:
	...
	breq minuto_max_A2
	rcall subtrai_Hexa_M
	rjmp seta_baixo_minuto_end_A2
minuto_max_A2:
	ldi r24, 0x59	
seta_baixo_minuto_end_A2:
	RCALL RTC_SET_M_A2
	ret
;-------------------------------------------------
;*************** seta_baixo_D_A ********************
...
seta_baixo_D_A_2:
	...
	breq dia_max_A2
	dec r26
	rjmp seta_baixo_dia_end_A2
dia_max_A2:
	ldi r26, 0x07	
seta_baixo_dia_end_A2:
	RCALL RTC_SET_D_A2
	rcall delay
	ret
;-------------------------------------------------
...
RTC_GET_ALARMS:
	...
	NOP
	RCALL	RTC_ON
	LDI		r18, 0x0F ;CMD Set minutes
	RCALL	SPI_TRANSFER
	LDI		r18, 0xFF ;DUMMY BYTE - RESPONSE MINUTO.
	RCALL	SPI_TRANSFER
	in		r16, SPDR ;Set Flag alarm 1 e 2 e habilita interrupção.
	;RCALL	SPI_TRANSFER
	RCALL	RTC_OFF
	RET
;------------------------------------
